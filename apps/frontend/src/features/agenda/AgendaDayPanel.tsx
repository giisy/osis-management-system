...
import { canManageAgenda, getCurrentRole } from '../../lib/permissions'
...
export default function AgendaDayPanel({ selectedDate, agendaList }: AgendaDayPanelProps) {
  const canManage = canManageAgenda(getCurrentRole())
        ...
        <div className="space-y-3">
          {agendaList.map((agenda) => {
            ...
            return canManage ? (
              ...
              <div key={agenda.id} className="p-3 rounded-lg border border-gray-100">
                {content}
              </div>
            )
          })}
